...
public class ActorsInFilms {
     ...
     *
     */
    public static class ActorsInFilmsMapper extends Mapper<Object, Text, Text, Text>{
        ...
        public void map(Object key, Text value, Context output)
            ...
            if(rawWords[4].equals("THEATRICAL_MOVIE")) {
                movie = rawWords[1] + "##" + rawWords[2] + "##" + rawWords[3];

                word.set(movie);
                actor.set(rawWords[0]);
                output.write(word, actor);
            }
    ...
    public static class ActorsInFilmsReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values,
            ...
            for(Text actor : values) {
                list = list + actor.toString() + "\t";
            }
            String str = list.substring(0, list.length()-1);
            output.write(key, new Text(str));
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (otherArgs.length != 2) {
            System.err.println("Usage: "+ActorsInFilms.class.getName()+" <in> <out>");
            System.exit(2);
        }
        ...
        job.setMapperClass(ActorsInFilmsMapper.class);
        job.setCombinerClass(ActorsInFilmsReducer.class); // in this case a combiner is possible!
        job.setReducerClass(ActorsInFilmsReducer.class);
